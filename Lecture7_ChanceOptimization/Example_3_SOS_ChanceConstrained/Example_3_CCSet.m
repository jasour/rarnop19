...
% set K
u=x; x1=q(1); x2=q(2); x3=q(3); w=q(4);
K=- 25*u^2 - 30*u*x1 + (25*u*x2)/2 - 25*u*x3 + 25*u - w^2*x2^2 - 2500*x1^2*x3^2 - 9*x1^2 + (15*x1*x2)/2 - 15*x1*x3 + 15*x1 - (25*x2^2)/16 + (25*x2*x3)/4 - (25*x2)/4 - (25*x3^2)/4 + (25*x3)/2 - 21/4;
...
WW=value(coef)'*(x(1).^vpow(:,1).*q(1).^vpow(:,2).*q(2).^vpow(:,3).*q(3).^vpow(:,4));

% obtained Integral of polynomial W(x,q) with respect to probability measure
%WW_Int=value(coef)'*(x(1).^vpow(:,1).*yq_1(vpow(:,2)+1));

WW_Int=value(coef)'*(x(1).^vpow(:,1).*yq_1(vpow(:,2)+1).*yq_2(vpow(:,3)+1).*yq_3(vpow(:,4)+1));
