...
[s4,c4] = polynomial([x1 x2],d);

% SDP Solver
ops = sdpsettings('solver','mosek');
% SOS Conditions
F = [sos(p-gamma-[s1 s2 s3 s4]*g), sos(s1), sos(s2), sos(s3),sos(s4)];
% Solve SOS based SDP
[sol,v,Q]=solvesos(F,-gamma,ops,[c1;c2;c3;c4;gamma]);
...
%S3: SOS sigma_3  
S4=sosd(F(5))'*sosd(F(5));
sdisplay(v{5}'*Q{5}*v{5})
sdisplay(sosd(F(5))'*sosd(F(5)))
...
clean(p-[S1 S2 S3 S4]*g - h'*h,1e-5)
