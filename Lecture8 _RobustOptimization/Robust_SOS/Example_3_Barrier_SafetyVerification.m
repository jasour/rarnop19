...
% UAV Dynamics
% states=[x,y,psi]: [position, yaw angle];  
% Dynamics: f1=v*sin(psi)+omega; f2=v*cos(psi); f3=u;
% v=1 m/s is the speed of the airplane
% omega: uncertainty: “cross-wind” (bounded between [-0.05, 0.05]). 
% control policy: states feedback : u=-K*(psi-psi_des) where psi_des is
% desired direction 

% tf: execution time of control policy
% y_0: initial states [x,y,psi]
tf=5; y0 = [0;0;0]; v=1; psi_des= 0*pi/180;
...
[V,c,Vm] = polynomial([x;y;psi],2*d);

% dV(x)/dt
dVdt = jacobian(V,[x;y;psi])*f;
...
% s1: SOS polynomial
[s1,c1] = polynomial([x;y;psi;omega],2*d);
% s2: SOS polynomial
[s2,c2] = polynomial([x;y;psi;omega],2*d);
% s3: SOS polynomial
[s3,c3] = polynomial([x;y;psi;omega],2*d);
...
L1=sdisplay(cc'*Vm);
% dV(x)/dt
dL1 = sdisplay(jacobian(cc'*Vm,[x;y;psi])*f);
...
[x,y,psi]=meshgrid([-1:0.01:1],[-1:0.01:1],[-1:0.01:1]);

p=patch(isosurface(x,y,psi,eval(L3),0));
 set(p,'FaceColor','red','EdgeColor','none','FaceAlpha',0.7);

p=patch(isosurface(x,y,psi,eval(L3),1));
...
xlabel('$x$','Interpreter','latex', 'FontSize',40);ylabel('$y$','Interpreter','latex', 'FontSize',40);zlabel('$\psi$','Interpreter','latex', 'FontSize',40);
